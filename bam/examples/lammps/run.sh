...
#
# Usage (new run):
#   bash run.sh                  # default: 1x1x1 (27 atoms), 1 GPU
#   bash run.sh 4                # 4x4x4
#   bash run.sh 4 3 2            # 4x3x2
#   bash run.sh 4 3 2 3          # 4x3x2, 3 GPUs
#
# Usage (restart from last checkpoint):
#   bash run.sh --restart 4      # restart 4x4x4
#   bash run.sh --restart 4 3 2  # restart 4x3x2
#
#   bash run.sh [--restart] RX [RY] [RZ] [NP]
...
# ── Output files ──────────────────────────────────────────────────
LOG="log_nvt_${TAG}.lammps"
OUTPUT="output_nvt_${TAG}.txt"
DUMP="dump_${TAG}.lammpstrj"
TRAJ="nvt_${TAG}.traj"

# ── Build LAMMPS args ────────────────────────────────────────────
LMP_ARGS="-in nvt_3bpa.in -var RX $RX -var RY $RY -var RZ $RZ -log $LOG"
...
if [ "$RESTART" -eq 1 ]; then
    ...
    echo "================================================================"
    echo " RESTART NVT: 3BPA C12H12N2O"
