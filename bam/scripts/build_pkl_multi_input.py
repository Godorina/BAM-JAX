"""Convert multiple .traj/.xyz files to .pkl graph files for multihead training.

Extended version of build_pkl_multihead.py that accepts multiple input files
per invocation, merging them into one pkl directory with continuous numbering.

Usage:
    # OMat24 replay (single file, built-in ATOM_ENERGIES)
    ...
    # Target train with multiple files (fit E0 from combined data)
    python -m bam.scripts.build_pkl_multi_input \
    ...
    # Target valid with multiple files (use fitted E0)
    python -m bam.scripts.build_pkl_multi_input \
        --input mptrj_valid.traj salex_valid.traj \
...
def build_pkl(
    ...
    Args:
        input_paths: Path or list of paths to .traj or .xyz files.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert .traj/.xyz to .pkl for multihead training (multi-input supported)")
    ...
    parser.add_argument("--fit-energies", action="store_true",
                        help="Fit per-element reference energies from the input data "
                             "(saves to atom_energies_target.json)")
    parser.add_argument("--atom-energies", type=str, default=None,
                        help="Path to atom_energies_target.json (from a previous --fit-energies run). "
    ...
    if args.atom_energies:
        ...
        # Fit atom energies from ALL input trajectories combined (chunked reading)
        ...
        # Pass 1: collect energies and element counts in chunks
        all_energies = []
        ...
        # Save to JSON
        save_path = "atom_energies_target.json"
        ae_data = {
